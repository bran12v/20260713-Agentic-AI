...
| # | Service | Job |
|---|---|---|
| 1 | Azure AI Foundry | Model deployments: a reasoning tier for the workers, a fast tier for classification and the readiness gate, an embedding model for the index, a multimodal deployment for reading the scanned intake form, and a judge deployment for § 13's evaluators |
...
3. **Images** — the multimodal deployment reads the scanned intake form in the context of the narrative and returns a typed corroboration verdict.
...
- **An analysts table and a grants table, seeded.** An entitlement is an analyst's grant over a partition of the records, and for this project the partition is the covered entity facility the incident happened at: every incident record carries a `facility_id`, and a grant is a `(analyst_id, facility_id)` row. Seed at least three analysts across at least three facilities, with one analyst holding two grants and one incident no one but its owner can read. Without those rows there is nothing for § 10's in-tool check to deny and nothing for the entitlement test in § 12 to assert.
...
- The affected population is **within the near-boundary margin of** 500 — an incident at 480 or 515 is genuinely in doubt about which channels R3 owes, and that is the case worth a human. A population far above the threshold is not: the outcome there is `presumed_breach`, `unsecured` and all three channels, which is the most protective answer the system can reach, arrived at against the favourable reading. Gating it adds a queue step to the one path that was already going to a person
- An access log or device inventory record contradicts the narrative

**Near-boundary margins** are configured per rule around both 500-individual boundaries and the 60-day boundary. R1 has no margin — the three exclusions are conditions, not quantities, and an exclusion that half applies does not apply. A margin is expressed in the boundary's own unit — days against a day count, individuals against a population, dollars against a dollar figure — never as a percentage of the boundary, which makes two margins on different scales look comparable when they are not. The chosen values are yours; record each one, with its unit and the reasoning, in the architecture document's decisions table.
...
Every case carries an id, the category from the table above, the query text, the expected outcome, the document ids and section paths that must appear in the answer's `sources` array, the subject `incident_id` where the case is incident-backed and null where it is not, and one line on why the case exists.
...
4. **Demonstration artifacts** — five of them, each a committed file rather than a live click-through, so a grader can check them without your laptop.
   ...
   - **The escalation contrast** — the `trace` and `dossier` output of the clean run, the same two from a run of the same incident with one field degraded, and two lines naming the trigger that fired and the queue row it produced. This is the artifact § 15 leans on hardest and the one most often submitted as a screenshot of a terminal that has since scrolled away.
...
- ☐ Every packet carries a date of the incident and a date of discovery, and every packet where they differ records how the discovery happened — R4's clocks run from discovery and never from the incident
...
- ☐ An analyst holding no grant over an incident's facility gets a structured denial from the tool, not an empty result set — seeded analysts, seeded grants, and a test that asserts both directions

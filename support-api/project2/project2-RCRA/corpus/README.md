# Cradle hazardous waste corpus

This directory contains excerpts of federal public-domain material — 40 CFR Parts 261, 262 and 268, the 2016 Generator Improvements Rule, EPA's guidance and interpretation letters on empty containers, and the Uniform Hazardous Waste Manifest with its instructions — reproduced as a training artifact for the Cradle exercise.

**It is not a current or authoritative source, and it is not environmental or legal guidance.** Sections have been excerpted out of their surrounding text, and the material is fixed at its 13 August 2026 retrieval date. Anyone with an actual waste determination to make must consult the published sources, which are linked per document in [MANIFEST.md](MANIFEST.md), and their authorised state programme, which is often stricter than the federal baseline.

The waste determinations the system analyses are fictional. Tarn Valley Manufacturing does not exist, and no EPA identification number in `packets/` belongs to a real facility.
...
python fetch_corpus.py CFR-261    # one document, no verification
```
...
`CFR-261`, `CFR-262` and `CFR-268` are pinned to eCFR issue date 2026-08-04 and will reproduce exactly. The EPA PDFs and the Federal Register document are served without version pinning, so if a rebuild produces different page counts, compare against `text/` to see what shifted before assuming the excerpt ranges in `sources.json` are still correct.

## A note on what this corpus leaves out

Part 261 is one of the largest parts in Title 40, and two of its sections could not be carried at any useful page budget: § 261.4, the exclusions, at 99,000 characters, and § 261.33, the P and U commercial chemical product lists, at 118,000. § 268.40, the treatment standards table, is 239,000 characters on its own — larger than this entire corpus.

The consequence is stated in [MANIFEST.md](MANIFEST.md) and matters when writing golden cases: **the corpus cannot resolve whether a given P-code or U-code is listed**, and a question that turns on one must be refused rather than answered. The acute-waste chain the exercise is built on runs through § 261.30(d) and § 261.31 instead, which name the acute F-codes in prose and are carried in full.

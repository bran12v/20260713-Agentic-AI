...
| # | Service | Job |
|---|---|---|
| 1 | Azure AI Foundry | Model deployments: a reasoning tier for the workers, a fast tier for classification and the readiness gate, an embedding model for the index, a multimodal deployment for reading container labels and log photographs |
...
- A session table holds the serialized transcript keyed by `(analyst_id, stream_id)`.
...
**Near-boundary margins** are configured per rule around the one-inch, 3-percent, 1 kg, 100 kg, 1,000 kg, 90-day, 180-day and 0.60 boundaries. R1 has no margin for list membership — an unrecognised code returns `insufficient_data`, not a default.
...
4. **Demonstration artifacts** — the escalation contrast (one stream clearing, the same stream with one signal degraded escalating) · indirect-injection resistance · the session-isolation test · the grounded-versus-ungrounded contrast · the MCP server driven from an external client.
...
- ☐ Every packet carries a generation date, an accumulation start date and a review date, and they differ
...
- ☐ Near-boundary margins configured per rule; an acute code, a near-boundary quantity, an R2 `not_empty` on a container the packet marked empty, or the two legs disagreeing all escalate
